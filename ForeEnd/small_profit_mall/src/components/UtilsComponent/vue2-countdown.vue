<template>
  <div>
    <div v-if="msTime.show">
      <!--        <span v-if="tipShow" class="se-info">{{tipText}}</span>-->
      <!--        <span v-if="!tipShow" class="se-info">{{tipTextEnd}}</span>-->
      <!--        <div class="se-hour">-->
      <!--          <span v-if="msTime.day>0"-->
      <!--                class="se-txt"><span>{{msTime.day}}</span><i>{{dayTxt}}</i></span>-->
      <!--        </div>-->
      <div class="se-hour">
        <span class="se-txt"><span>{{msTime.hour}}</span><i>{{hourTxt}}</i></span>
      </div>
      <div class="se-hour">
        <span class="se-txt"><span>{{msTime.minutes}}</span><i>{{minutesTxt}}</i></span>
      </div>
      <div class="se-hour">
        <span class="se-txt"><span>{{msTime.seconds}}</span><i>{{secondsTxt}}</i></span>
      </div>
    </div>
    <!--    <p v-if="!msTime.show" class="se-info">{{endText}}</p>-->
...
<script>
  export default {
    ...
    methods: {
      gogogo: function () {
        //判断是秒还是毫秒
        this.startTime.toString().length == 10 ? this.star = this.startTime * 1000
            : this.star = this.startTime;
        this.endTime.toString().length == 10 ? this.end = this.endTime * 1000
            : this.end = this.endTime;
        if (this.currentTime) {
          this.currentTime.toString().length == 10 ? this.current = this.currentTime * 1000
        ...
        } else if (this.end > this.current && this.star < this.current || this.star
            == this.current) {
          /**
