...
public class testJson {
    public static void main(String[] args) {
        ...
        if ("success".equals(desc)) {
            ...
            List<List<String>> dateDataList = new ArrayList();
            for (int i = 0; i < total; i++) {
                List dateLists = (List) dateList.get(i);
                List dataLists = (List) dataList.get(i);

                List<String> dateDataArray = new ArrayList();
                String ad = dateLists.get(0).toString();
                String ads = dataLists.get(0).toString();
                System.out.println(ad);
                System.out.println(ads);
                dateDataArray.add(ad);
                dateDataArray.add(ads);
